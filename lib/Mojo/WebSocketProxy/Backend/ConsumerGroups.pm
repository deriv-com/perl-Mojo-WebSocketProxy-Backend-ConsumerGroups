...
sub call_rpc {
    ...
    $self->request($request_data, $req_storage->{category}, $category_timeout)->then(
        sub {
            ...
            my $result = MojoX::JSON::RPC::Client::ReturnObject->new(rpc_response => +{ result => $message->{response} });
...
sub _prepare_request_data {
    ...
    my $request_data = [
        ...
        $params->{args}  ? (args  => encode_json_utf8($params->{args}))  : (),
        $params->{stash} ? (stash => encode_json_utf8($params->{stash})) : (),
    ];
...
sub make_call_params {
    ...
    if (defined $stash_params) {
        $stash->{$_} = $c->stash($_) for $stash_params->@*;
    }

    if ($req_storage->{logger}) {
        $stash->{req_log_context} = $req_storage->{logger}->get_context();
    }
